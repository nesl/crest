...
/**
  ...
  *
  * Local changes:
  *   - Replaced the stock example loop with CREST DUT bootstrap and runtime
  *     control flow.
  ...
  *   - Added CREST phase-config and DUT-runner integration.
  *
  * Licensing note:
...
static void DebugComClock_Init(void);
static void DebugCom_Init(void);
/* USER CODE END PFP */
...
int main(void)
{
  ...
  /* Configure MPU before enabling caches so the noncacheable region is honored. */
  MPU_Config();
...
#if CREST_DUT_CPU_CLOCK_MHZ != 800
...
#else
#if CREST_DUT_CPU_CLOCK_MHZ == 600
  ic1_divider = 2U;
#elif CREST_DUT_CPU_CLOCK_MHZ == 400
  ic1_divider = 3U;
#elif CREST_DUT_CPU_CLOCK_MHZ == 300
  ic1_divider = 4U;
#elif CREST_DUT_CPU_CLOCK_MHZ == 200
  ic1_divider = 6U;
#else
#error "Unsupported CREST_DUT_CPU_CLOCK_MHZ preset"
#endif
