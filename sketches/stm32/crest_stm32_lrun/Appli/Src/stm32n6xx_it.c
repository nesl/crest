...
/**
  ...
  *
  * Local changes:
  *   - Added CREST fault-reporting traps that emit debug COM diagnostics
  *     before halting.
  *   - Added DUT-runner and XSPI integration needed by the CREST LRUN app.
...
static void fault_trap(const char *label)
{
  printf("%s\r\n", label);
  fflush(stdout);
  while (1)
...
void HardFault_Handler(void)
{
  ...
  fault_trap("STM32_FAULT=HardFault");
}
...
void MemManage_Handler(void)
{
  ...
  fault_trap("STM32_FAULT=MemManage");
}
...
void BusFault_Handler(void)
{
  ...
  fault_trap("STM32_FAULT=BusFault");
}
...
void UsageFault_Handler(void)
{
  ...
  fault_trap("STM32_FAULT=UsageFault");
}
...
void SecureFault_Handler(void)
{
  ...
  fault_trap("STM32_FAULT=SecureFault");
}
...
{
  HAL_XSPI_IRQHandler(&hxspi_nor[1]);
}
