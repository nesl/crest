...
static void emit_measurement_telemetry(const TcnDutMeasurement *measurement)
{
  ...
  if ((TCN_DUT_SELECTED_PHASE == TCN_DUT_PHASE_CADENCED) && (measurement->runs_completed > 0U))
  {
  ...
  emit_signed_us_line("wake recovery us",
                      ...
                      (TCN_DUT_SELECTED_PHASE == TCN_DUT_PHASE_CADENCED) && (measurement->runs_completed > 0U));
