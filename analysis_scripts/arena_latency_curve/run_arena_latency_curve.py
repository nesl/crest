...
def _resolve_settings(args: argparse.Namespace, server: Any, runtime_device: Any) -> SweepSettings:
    ...
    default_stem = SCRIPT_DIR / "results" / f"arena_latency_curve_{device_name}_{core_label}_{timestamp_tag}"
...
def main() -> int:
    ...
    for arena_idx, arena_kb in enumerate(settings.arena_kb_list):
        for repeat_idx in range(1, settings.repeats + 1):
            attempt_index += 1
            logging.info(
                "Attempt %d/%d: arena=%d KiB repeat=%d",
                attempt_index,
                total_attempts,
                arena_kb,
                repeat_idx,
            ...
            try:
                ...
                logging.exception(
                    "Attempt failed via exception (arena=%d KiB, repeat=%d): %s",
                    arena_kb,
                    repeat_idx,
                    exc,
                )
            ...
            logging.info(
                "Attempt result: arena=%d KiB repeat=%d error=%s latency_ms=%.3f energy_mj=%.3f",
                arena_kb,
                repeat_idx,
                record["error_label"],
    ...
    title = (
        f"Arena Latency/Energy Curve - {settings.device_name}"
