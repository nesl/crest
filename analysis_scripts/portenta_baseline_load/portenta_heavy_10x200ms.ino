// Portenta baseline heavy-workload sketch:
// 10 iterations, each busy for ~200 ms, measured as one harness window.
#include <Arduino.h>
...
constexpr uint32_t kIterationBudgetUs = 200000UL;  // 200 ms
...
void RunHeavyIteration() {
  const uint32_t iter_start_us = micros();
  while (micros() - iter_start_us < kIterationBudgetUs) {
    // Keep the core active with simple floating-point work.
    g_sink += 1.000001f;
    g_sink *= 0.999999f;
    if (g_sink > 100000.0f) {
      g_sink = 0.0f;
