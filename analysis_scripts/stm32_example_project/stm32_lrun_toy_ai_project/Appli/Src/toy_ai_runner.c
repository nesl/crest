...
#ifndef TOY_AI_USE_STUB
#define TOY_AI_USE_STUB 0
#endif
...
#if TOY_AI_USE_STUB
static void populate_stub_measurement(ToyAiMeasurement *measurement)
{
  uint64_t per_inference_scaled_1e6 = 1000ULL;
  uint64_t per_window_scaled_1e6 = per_inference_scaled_1e6 * (uint64_t)TOY_AI_MEASURED_RUNS;

  measurement->clock_hz = read_cpu_clock_hz();
  measurement->runs_completed = TOY_AI_MEASURED_RUNS;
  measurement->have_cycles = true;
  measurement->have_timing = true;
  measurement->deadline_miss_count = 0U;

  if (measurement->clock_hz > 0U)
  {
    measurement->dwt_cycles_per_inference_milli =
        ((uint64_t)measurement->clock_hz * per_inference_scaled_1e6) / 1000ULL;
  }
  else
  {
    measurement->dwt_cycles_per_inference_milli = 0ULL;
  }

  measurement->timer_per_inference_scaled_1e6 = per_inference_scaled_1e6;
  if (TOY_AI_SELECTED_PHASE == TOY_AI_PHASE_CADENCED)
  {
    per_window_scaled_1e6 = (uint64_t)TOY_AI_LATENCY_BUDGET_MS * 1000ULL * (uint64_t)TOY_AI_MEASURED_RUNS;
  }
  measurement->timer_per_window_scaled_1e6 = per_window_scaled_1e6;
  measurement->rtc_sleep_total_us = 0ULL;
  measurement->wake_recovery_total_us = 0LL;
  measurement->wake_overshoot_total_us = 0LL;
}
#endif
...
int toy_ai_run_once(void)
{
  ...
  if (!rtc_init())
  {
    emit_line("STM32_AI_INIT=FAIL stage=rtc_init");
  ...
  populate_stub_measurement(&measurement);
  ...
  {
    emit_line("STM32_AI_INIT=FAIL stage=external_weight_mapping");
    return -5;
  }
