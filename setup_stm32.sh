...
CANONICAL_LRUN_TEMPLATE_ROOT="$PROJECT_ROOT/sketches/stm32/crest_stm32_lrun"
...
load_lrun_ownership_manifest() {
  ...
  while IFS=$'\t' read -r category relative_path source_path; do
    ...
    case "$category" in
      vendor_copy|vendor_derived|crest_owned|generated|build_recipe)
        ;;
      *)
  ...
  for relative_path in "${LRUN_MANIFEST_PATHS[@]}"; do
    ...
    case "$category" in
      vendor_copy)
        ...
        ;;
      vendor_derived|crest_owned|build_recipe)
...
record_lrun_materialized_files() {
  ...
  while IFS= read -r file_path; do
    ...
    category="${LRUN_CATEGORY_BY_PATH[$relative_path]:-}"
    case "$category" in
      crest_owned|vendor_derived|build_recipe|generated)
        continue
        ;;
    esac
