...
def build_odom_tcn_model(hyperparams: dict[str, Any]) -> Model:
    ...
    # Preserve the original Odom TCN post-processing stack so the family keeps
...
def apply_combined_perturbation(
    ...
    """Apply the CREST deterministic BN/bias perturbation pass.
...
class OdomTCNFamily(ModelFamilyABC):
    """Describe tCN model family matching the current CREST architecture surface.

    The family exposes the stable ``"odom_tcn"`` name, owns the CREST
    ...
    def sample_hparams(
        ...
        """Sample the current CREST NAS hyperparameter surface.
    ...
    def decode_trial_hparams(
        ...
        """Decode persisted Optuna trial parameters for the CREST family.
        ...
        -------
        dict[str, Any]
            Build-time CREST hyperparameters with ``dilations`` expanded.
        """
        del ctx, config
    ...
    def default_seed_trial(
        ...
        """Return the legacy baseline CREST seed payload.
    ...
    def build_model(
        ...
        return build_odom_tcn_model(build_hparams)
    ...
    def materialize_export_model(
        ...
        *,
        ...
        """Materialize one CREST export model variant.
        ...
        if normalized_variant in {
            ...
            logger.info(
                "Materialized CREST export variant '%s' with deterministic perturbation "
