...
def _patch_sketch_constants(
    ...
    """Rewrite CREST deployment constants inside the Arduino sketch.
    ...
    latency_budget_ms : float | None, optional
        ...
        sketch declares ``CREST_LATENCY_BUDGET_MS``, the define is updated
        in-place.
    ...
    text = _replace_define(text, "CREST_WINDOW_SIZE", str(window_size))
    text = _replace_define(text, "CREST_NUM_CHANNELS", str(num_channels))
    text = _replace_define(text, "CREST_TENSOR_ARENA_BYTES", f"({arena_kb} * 1024)")
    if latency_budget_ms is not None and "CREST_LATENCY_BUDGET_MS" in text:
        text = _replace_define(
            text,
            "CREST_LATENCY_BUDGET_MS",
...
def measure_serial(
    *,
    ...
    harness_arm_timeout_s : float
        ...
        and compiled into the harness firmware (`CREST_HARNESS_ARM_TIMEOUT_MS`).
    ...
    harness_active_timeout_s : float
        ...
        (`CREST_HARNESS_ACTIVE_TIMEOUT_MS`).
    ...
    build_defines = {
        ...
        "CREST_HARNESS_ARM_PIN": int(harness_arm_pin),
        "CREST_HARNESS_TRIGGER_PIN": int(harness_trigger_pin),
        "CREST_DUT_ARM_HOLD_MS": int(dut_arm_hold_ms),
        "CREST_HARNESS_STABLE_LOW_MS": int(harness_stable_low_ms),
        "CREST_HARNESS_ARM_TIMEOUT_MS": harness_arm_timeout_ms,
        "CREST_HARNESS_ACTIVE_TIMEOUT_MS": harness_active_timeout_ms,
    }
